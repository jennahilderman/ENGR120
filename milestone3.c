...
int findPosition(){
    ...
    if (SensorValue[infraLeft] < IR_SENSOR_THRESHOLD || SensorValue[infraBack] < IR_SENSOR_THRESHOLD){
        position = left;
        return 2;
        }
    if (SensorValue[infraFrontR] > IR_SENSOR_THRESHOLD && SensorValue[infraFrontL] > IR_SENSOR_THRESHOLD && SensorValue[infraRight] > IR_SENSOR_THRESHOLD && SensorValue[infraBack] > IR_SENSOR_THRESHOLD && SensorValue(batInput) == tooFar){
        position = wall;
        return 3;
    }
    position = straight;
...
void lightLED(){
    if(SensorValue(batInput) > justRight && findPosition() == 0){
